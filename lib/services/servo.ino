  ...
  switch (currentState) {
    ...
    case STATE_REED_CHECK:
      if (reedSwitchOpen) {
        ...
        // Randomly pick one of three angles: 0, 70, or 180
        int angles[] = {0, 70, 180};
        servo1RandomAngle = angles[random(0, 3)];
        
        Serial.print("Servo 1 picked angle: "); Serial.println(servo1RandomAngle);
    ...
    case STATE_SERVO_1_SORTING: {
      Serial.print("Servo 1: Moving to "); Serial.print(servo1RandomAngle); Serial.println("°");
      
      // Move directly without stepping loop
      servo1.write(servo1RandomAngle);
      delay(500); // Allow time to reach position

      Serial.println("✓ Servo 1 reached position");
    ...
    case STATE_SERVO_1_CLOSING:
      if (now - stateStartTime >= SERVO_1_CLOSE_DELAY) {
        Serial.println("Servo 1: Returning to 75°");
        
        // Move directly without stepping loop
        servo1.write(75);
        delay(500);

        Serial.println("✓ Servo 1 returned to 75°");
    ...
    case STATE_SERVO_3_DUMPING: {
      ...
      Serial.println("\n>>> SEQUENCE COMPLETE — WAITING FOR TRIGGER OFF <<<");
    ...
    case STATE_COOLDOWN:
      // Wait for the external trigger to go LOW before resetting to idle.
      // This prevents immediate re-triggering if the button/signal is still held.
      if (!externalTriggerActive && (now - stateStartTime >= COOLDOWN_TIME)) {
        resetToIdle();
      }
      break;
  }
...
void connectWiFi() {
  ...
  // FIX: Disabled static IP - was causing DNS resolution to return 0.0.0.0
  // Use DHCP instead. Re-enable only if DHCP works and you need a fixed IP.
  /*
