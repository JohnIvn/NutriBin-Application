...
String WIFI_SSID_2     = "Luke";
String WIFI_PASSWORD_2 = "chananana";
...
void connectWiFi() {
  ...
  // FIX: Try DHCP first - static IP was causing DNS issues (0.0.0.0 resolution)
  // Uncomment the static IP block below ONLY if DHCP works and you need a fixed IP
  /*
  ...
  */
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (WiFi.hostByName("google.com", testIP)) {
      Serial.print("  DNS test (google.com): ");
      Serial.println(testIP);
    } else {
      Serial.println("  WARNING: DNS test failed!");
    }
  } else {
...
// Debug: test if we can reach the server
bool testServerConnectivity() {
  Serial.println("Testing server connectivity...");
  
  // Extract hostname from URL
  String host = "nutribin-server-backend-production.up.railway.app";
  
  // Test DNS resolution
  IPAddress serverIP;
  if (!WiFi.hostByName(host.c_str(), serverIP)) {
    Serial.println("DNS FAILED - cannot resolve: " + host);
    return false;
  }
  Serial.print("DNS OK - resolved to: ");
  Serial.println(serverIP);
  
  // Test raw TCP connection on port 443
  WiFiClient testClient;
  testClient.setTimeout(10);
  if (!testClient.connect(host.c_str(), 443)) {
    Serial.println("TCP FAILED - cannot connect to port 443");
    return false;
  }
  Serial.println("TCP OK - port 443 reachable");
  testClient.stop();
...
int doPost(const String &url, const String &payload) {
  // Quick connectivity check on repeated failures
  static int consecutiveFailures = 0;
  if (consecutiveFailures >= 3) {
    Serial.println("Multiple failures - running connectivity test...");
  ...
  HTTPClient http;
  http.setTimeout(20000);    // 20 second timeout for HTTP operations
  http.setConnectTimeout(10000);  // 10 second connection timeout

  if (!http.begin(client, url)) {
    Serial.println("HTTP begin() failed - invalid URL or memory issue");
    consecutiveFailures++;
    return -1;
  }
...
void printData() {
  Serial.println(
    ...
    " H:" + String(currentData.humidity_percent, 1) +
...
// ================= WEB SERVER =================
void handleRestart() {
  server.send(200, "application/json", "{\"ok\": true, \"message\": \"Restarting ESP32...\"}");
  delay(500);
  ESP.restart();
}
...
<body>
  ...
  <p style='color:#00d4ff; font-weight:bold;'>Local IP: )" + WiFi.localIP().toString() + R"rawhtml(</p>
)rawhtml";
